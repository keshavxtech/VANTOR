...
import { CreateEvaluationInput, EvaluationStatus } from "@/types/evaluation";
import { X, Activity, Cpu, Database } from "lucide-react";

interface RunEvaluationModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (input: CreateEvaluationInput) => void;
}

export function RunEvaluationModal({ isOpen, onClose, onAdd }: RunEvaluationModalProps) {
  const [name, setName] = useState("");
  const [description, setDescription] = useState("");
  const [modelName, setModelName] = useState("Claude Sonnet 5");
  const [datasetName, setDatasetName] = useState("AST-Refactor-Eval-Suite");
  const [status, setStatus] = useState<EvaluationStatus>("passed");
      ...
      <div className="w-full max-w-lg rounded-[14px] bg-[#161B22] border border-white/[0.08] shadow-[0_8px_32px_rgba(0,0,0,0.8)] overflow-hidden">
        <div className="flex items-center justify-between p-5 border-b border-white/[0.06]">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-sans font-semibold text-white">Trigger Evaluation Suite</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4 text-xs font-sans">
          <div className="space-y-1">
            <label className="text-[#8B95A5] font-medium">Evaluation Suite Name *</label>
            <input
              type="text"
              required
              placeholder="e.g. AST Refactor Accuracy & Safety Suite"
              value={name}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              <label className="text-[#8B95A5] font-medium">Target Model</label>
              <select
                value={modelName}
            ...
            <div className="space-y-1">
              <label className="text-[#8B95A5] font-medium">Test Dataset Benchmark</label>
              <select
                value={datasetName}
          ...
          <div className="space-y-1">
            <label className="text-[#8B95A5] font-medium">Evaluation Objectives & Guardrails</label>
            <textarea
              rows={3}
              placeholder="Describe assertions, threshold pass rates, and judge LLM criteria..."
              value={description}
